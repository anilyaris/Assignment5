...
Vec3f mirrorDirection(const Vec3f& normal, const Vec3f& incoming) {
	...
	return (incoming - 2 * normal.dot(incoming) * normal).normalized();	
}
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float refr_index, Hit& hit, FW::Vec3f debug_color) const {
	...
	Vec3f light = scene_.getAmbientLight();
	Vec3f answer = light * m->diffuse_color(point) /**max(0.0f, FW::dot(normal.normalized(), scene_.getAmbientLight().normalized())) (light - 2.0f * light.dot(normal) * normal)*/;
	...
	int numLights = scene_.getNumLights();
	for (int i = 0; i < numLights; i++) {
		Light* light = scene_.getLight(i);
		Vec3f dir_to_light = Vec3f(0, 0, 0), incident_intensity = Vec3f(0, 0, 0);
		float distance = 0;
		light->getIncidentIllumination(point, dir_to_light, incident_intensity, distance);
		bool blocked = false;
		if (args_.shadows) {
			Ray shadow = Ray(point, dir_to_light);
			blocked = scene_.getGroup()->intersect(shadow, Hit(), 0.001f);			
		}
		if (!blocked) answer += m->shade(ray, hit, dir_to_light, incident_intensity, false);
	}
	...
	if (bounces >= 1) {
		...
		if (m->reflective_color(point).length() > 0.0f) {
			...
			Vec3f reflected_color = traceRay(reflected, 0.001f, bounces - 1, refr_index, Hit(), debug_color);
			answer += reflected_color * m->reflective_color(point);
		}
