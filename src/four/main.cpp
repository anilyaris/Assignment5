...
GLuint render(RayTracer& ray_tracer, SceneParser& scene, const Args& args) {
	...
	for (int j = 0; j < args.height; ++j) {
		...
		for (int i = 0; i < args.width; ++i) {
			...
			Vec3f sample_color = Vec3f(0, 0, 0);
			float depth = 0;
			Vec3f normal_color = Vec3f(0, 0, 0);
			...
			for (int n = 0; n < args.num_samples; ++n) {
				...
				if (depth_image) {
					...
					float f = hit.t;
					if (f > args.depth_max) f = args.depth_max;
					else if (f < args.depth_min) f = args.depth_min;
					depth += (f - args.depth_max) / (args.depth_min - args.depth_max);					
				}
				if (normals_image) {
					Vec3f n = hit.normal;
					Vec3f color(fabs(n[0]), fabs(n[1]), fabs(n[2]));
					normal_color += color.clamp(Vec3f(0), Vec3f(1));
					normals_image->setVec4f( Vec2i( i, j ), Vec4f( color, 1 ) );
				}
			}
			sample_color /= args.num_samples;
			depth /= args.num_samples;
			normal_color /= args.num_samples;

			image->setVec4f(Vec2i(i, j), Vec4f(sample_color, 1));
			if (depth_image) depth_image->setVec4f(Vec2i(i, j), Vec4f(Vec3f(depth), 1));
			if (normals_image) normals_image->setVec4f(Vec2i(i, j), Vec4f(normal_color, 1));
		}
		++lines_done;
	}
