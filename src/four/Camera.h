...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		return FW::Vec2f(2 * pixel.x / imageSize.x - 1, -(2 * pixel.y / imageSize.y - 1));		
	}
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		//return Ray(FW::Vec3f(point * size / 2, 0) + center, direction);
		return Ray(center + size / 2 * point.x * horizontal + size / 2 * point.y * up, direction);
	}
...
public:
	...
	{	
		...
		float d = 1 / FW::tan(fov_angle / 2);
		//return Ray(FW::Vec3f(0, 0, center.z), FW::Vec3f(point, -d).normalized());
		return Ray(center, (point.x * horizontal + point.y * up + d * direction).normalized());
	}
